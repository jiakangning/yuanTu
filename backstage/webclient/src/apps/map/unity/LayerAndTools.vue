  ...
  <div :class="['surveyingTools', activeName]">
    ...
    <ul class="toolButtons">
      ...
      <li @click="toggleShow('Tools')" :class="{active: activeName === 'Tools'}">
        <div class="rotate45">
          <i class="iconfont icon-straighten"></i>
        </div>
      </li>
    </ul>
    <!-- 工具面板 -->
    <div :class="['panel', activeName]">
      <component :is="activeName" v-if="activeName" :divHeight="divHeight" @changeLayerVisible="changeLayerVisible" @loadLayer="loadLayer" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    changeLayerVisible({data, visible, contains}) {
      this.$emit('changeLayerVisible', {data, visible, contains})
    },
    loadLayer() {
      this.$emit('loadLayer')
    }
