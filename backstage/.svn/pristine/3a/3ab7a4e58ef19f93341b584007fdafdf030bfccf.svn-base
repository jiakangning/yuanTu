<script type="text/jsx">
  ...
  export default {
    ...
    computed: {
      ...
      currentTotalKey() {
        return this.options.getConfig('count')
      },
      currentTotal() {
        return this.options.getSources(this.currentTotalKey)
      },
      currentParams() {
        let obj = this.currentPagination
          ? { pagination: true, pageSize: this.currentLimit, currentPage: this.page }
          : { pagination: false }
        return Object.assign({}, obj, this.sort)
      }
    },
    data() {
      return {
